// GPU runtime: direct NHWC 2x2 convolution for the medium detector stem branches.
...
// 9x9 input patch with 16 channels, plus 2x2x16x32 weights.
var<workgroup> input_tile: array<f32, 1296>;
...
fn conv_2x2_direct(
    ...
    for (var input_channel_base = 0u;
        ...
        input_channel_base += 16u) {
        var input_index = local_linear;
        loop {
            if input_index >= 1296u { break; }
            let patch_point = input_index / 16u;
            let patch_channel = input_index - patch_point * 16u;
            let patch_y = patch_point / 9u;
            let patch_x = patch_point - patch_y * 9u;
            let input_y = tile_y * 8u + patch_y;
            let input_x = tile_x * 8u + patch_x;
            var value = 0.0;
            if input_y < params.input_height && input_x < params.input_width {
                value = arena[params.input_offset
                    + (((batch_index * params.input_height + input_y) * params.input_width
                    + input_x) * params.input_channel_stride)
        ...
        loop {
            ...
            let kernel_spatial = local_k / 16u;
            let input_channel = input_channel_base + local_k - kernel_spatial * 16u;
        ...
        for (var kernel_y = 0u; kernel_y < 2u; kernel_y += 1u) {
            for (var kernel_x = 0u; kernel_x < 2u; kernel_x += 1u) {
                let input_base = ((local_id.y + kernel_y) * 9u
                    + local_id.x + kernel_x) * 16u;
                let weight_base = (kernel_y * 2u + kernel_x) * 128u + channel4_base;
                for (var input_channel = 0u; input_channel < 16u; input_channel += 1u) {
