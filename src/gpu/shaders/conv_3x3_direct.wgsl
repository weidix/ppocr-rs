// GPU runtime: direct NHWC 3x3 convolution for the medium detector head.
...
var<workgroup> weight_tile: array<vec4<f32>, 576>;
...
fn conv_3x3_direct(
    ...
    loop {
        if input_channel_base >= params.input_channels { break; }

        var input_index = local_linear;
        loop {
            if input_index >= 800u { break; }
            let patch_point = input_index / 8u;
            let patch_channel = input_index - patch_point * 8u;
            let patch_y = patch_point / 10u;
            let patch_x = patch_point - patch_y * 10u;
            let input_y = i32(tile_y * 8u + patch_y) - 1;
            let input_x = i32(tile_x * 8u + patch_x) - 1;
            var value = 0.0;
        ...
        loop {
            if weight_index >= 576u { break; }
            let local_k = weight_index / 8u;
            let channel4 = weight_index - local_k * 8u;
            let kernel_spatial = local_k / 8u;
            let input_channel = input_channel_base + local_k - kernel_spatial * 8u;
        ...
        for (var kernel_y = 0u; kernel_y < 3u; kernel_y += 1u) {
            for (var kernel_x = 0u; kernel_x < 3u; kernel_x += 1u) {
                let input_base = ((local_id.y + kernel_y) * 10u
                    + local_id.x + kernel_x) * 8u;
                let weight_base = (kernel_y * 3u + kernel_x) * 64u + channel4_base;
                for (var input_channel = 0u; input_channel < 8u; input_channel += 1u) {
