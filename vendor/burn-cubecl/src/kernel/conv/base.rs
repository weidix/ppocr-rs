...
pub enum ConvStrategy {
    ...
    Direct,
    /// Uses the matmul path for compatible ungrouped 1x1 convolutions and the direct path
...
impl Default for ConvStrategy {
    fn default() -> Self {
        ...
        // The 1x1 path is valid for compatible shapes and falls back to Direct everywhere else.
        #[cfg(not(feature = "autotune"))]
        ConvStrategy::Im2Col1x1
    }
}
...
pub fn conv_forward_nhwc<R: CubeRuntime, const N: usize>(
    ...
    match strategy {
        ...
        ConvStrategy::Im2Col1x1 => {
            if can_use_im2col_1x1(&input, &weight, &options) {
...
fn can_use_im2col_1x1<R: CubeRuntime, const N: usize>(
    input: &CubeTensor<R>,
    weight: &CubeTensor<R>,
    options: &ConvOptions<N>,
) -> bool {
    ...
    let channel_dim = rank - 1;
    let input_shape = &input.meta.shape()[1..channel_dim];
    let kernel_shape = &weight.meta.shape()[1..channel_dim];

    kernel_shape.iter().all(|size| *size == 1)
...
pub fn conv_weight_backward<R: CubeRuntime, const N: usize>(
    ...
    let weight_grad = match strategy {
        ConvStrategy::Direct | ConvStrategy::Im2Col1x1 => {
